...
if(mysql_num_rows($rsSvr) > 0)
{
	...
	if($HTTP_GET_VARS[a]=='1')
	{
		...
		if($ret == 255) die("<font color=red>ZoneServer, $zs, is not setup for remote game admin.</font><p><a href='scene_test.php'>Back</a>");
		...
		if ($scene_id >= 150 && $scene_id <=199)
		{
            // Update the SceneID
            $qry_sceneid = mysql_query("SELECT * FROM pcharacter WHERE SceneID >= 150 AND SceneID <= 199",$dbGmAdm) or die(mysql_error());
            $no_player = mysql_num_rows($qry_sceneid);

            $qry_avail_scene = mysql_query("SELECT * FROM scene WHERE (SceneID >=150 AND SceneID <= 199) AND AccessFlag=1",$dbGmAdm) or die(mysql_error());
            ...
            {
                $blah = mysql_query("SELECT * FROM pcharacter WHERE SceneID >= 150 AND SceneID <= 199 LIMIT $offset2, $total_row",$dbGmAdm) or die(mysql_error());
                while ($rw3 = mysql_fetch_assoc($blah))
                {
                    $run_update = mysql_query("UPDATE pcharacter SET SceneID = $rw2[SceneID] WHERE CharID= $rw3[CharID]", $dbGmAdm) or die(mysql_error($dbGmAdm));
                }
                $offset2 = $offset2 + $total_row;
            }
            
            // Update the BindSceneID
            $qry_bindsceneid = mysql_query("SELECT * FROM pcharacter WHERE BindSceneID >= 150 AND BindSceneID <= 199",$dbGmAdm) or die(mysql_error());
            $no_bindsceneplayer = mysql_num_rows($qry_bindsceneid);

            $qry_avail_scene2 = mysql_query("SELECT * FROM scene WHERE (SceneID >=150 AND SceneID <= 199) AND AccessFlag=1",$dbGmAdm) or die(mysql_error());
            $no_scene = mysql_num_rows($qry_avail_scene2);

            $each = $no_bindsceneplayer / $no_scene;

            $offset2 = 0;
            $total_row = ceil($each);
            while ($rw2 = mysql_fetch_assoc($qry_avail_scene2))
            {
                $blah = mysql_query("SELECT * FROM pcharacter WHERE BindSceneID >= 150 AND BindSceneID <= 199 LIMIT $offset2, $total_row",$dbGmAdm) or die(mysql_error());
                while ($rw3 = mysql_fetch_assoc($blah))
                {
                    $run_update = mysql_query("UPDATE pcharacter SET BindSceneID = $rw2[BindSceneID] WHERE CharID= $rw3[CharID]", $dbGmAdm) or die(mysql_error($dbGmAdm));
                }
                $offset2 = $offset2 + $total_row;
            }
        }
		...
		$after = join("\n", $shell_out);
		if($ret != 0) die("<font color=red>Failed to setcmd to server, $zs.</font><p><a href='scene_test.php'>Back</a>");
		act_log(
			array(
				"server"=>"$zs",
				"db"=>"",
		...
		header("Location: scene_test.php");
		exit();
	}
...
$htmlWc="<select name=wid onchange=\"postform(document.form1,'scene_test.php')\"><option value=''></option>";
foreach($arWc as $row)
{
...
if(is_resource($rsScene))
{
	while($row=mysql_fetch_assoc($rsScene))
	{
		...
		if($row[AccessFlag])
		{
			$toggleflag = "<input type=button value=\"Disable\" onclick=\"if(confirm('Confirm disable?'))postform(document.form1, 'scene_test.php?a=0&i={$row[SceneID]}&ip={$row[Address]}')\">";
		}
		else
		{
			$toggleflag = "<input type=button value=\"Enable\" onclick=\"if(confirm('Confirm enable?'))postform(document.form1, 'scene_test.php?a=1&i={$row[SceneID]}&ip={$row[Address]}')\">";
		}
