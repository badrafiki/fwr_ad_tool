...
else
{
	...
	$skilllist_table = "skilllist_" . ($HTTP_GET_VARS[f] % 10);
	...
	if($HTTP_GET_VARS[a]=="s")
	{
		...
		if($HTTP_POST_VARS["ids"])
		{
			...
			foreach($indxes as $indx)
			{
				...
				$skillid=$HTTP_POST_VARS["skillid_$indx"];
				$rank=$HTTP_POST_VARS["rank_$indx"];

				$query_rs = "UPDATE $skilllist_table SET
					SkillID='{$skillid}',
					Rank='{$rank}'
					WHERE Indx='$indx'";

				$befores = get_str_rs($dbWc, "SELECT p.Username, p.CharacterName, s.* FROM $skilllist_table s, pcharacter p WHERE Indx='$indx' AND p.CharID=s.CharID;");
				$rs = mysql_query($query_rs, $dbWc) or die(mysql_error($dbWc));
				$after = get_str_rs($dbWc, "SELECT p.Username, p.CharacterName, s.* FROM $skilllist_table s, pcharacter p WHERE Indx='$indx' AND p.CharID=s.CharID;");
				
			}
		}
		header("Location: skilllist.php?f={$HTTP_GET_VARS[f]}&wid=$wid");
		exit();
	}

	$query_rs = "SELECT * FROM $skilllist_table WHERE CharID='{$HTTP_GET_VARS[f]}' ORDER BY SkillID";
...
<body>
<h3>Player Character Skill List</h3>
...
?>
	<tr>
		...
		<td><input name="skillid_<?=$row[Indx]?>" type="text" id="skillid_<?=$row[Indx]?>" value="<?=$row[SkillID]?>" size="8" onchange="saveid(<?=$row[Indx]?>)"><!--a href="skills.php?i=<?=$row[SkillID]?>&wid=<?=$wid?>"--><?=getstring($row[SkillID],'skill')?><!--/a--></td>
		<td>
			<input name="rank_<?=$row[Indx]?>" type="text" id="rank_<?=$row[Indx]?>" value="<?=$row[Rank]?>" size="8" onchange="saveid(<?=$row[Indx]?>)">
			<input type="hidden" name="mark_<?=$row[Indx]?>" value="">
		</td>
		<!--td><input type="button" name="Button" value="Save" onClick="if(confirm('Overwrite?'))postform(document.form1,'skilllist.php?a=s&f=<?=$HTTP_GET_VARS[f]?>&i=<?=$row[Indx]?>&wid=<?=$wid?>')"></td-->
	</tr>
    <?php
}
?>
...
<input type="reset" value="Reset"> <input type="button" name="Button" value="Save" onClick="if(confirm('Overwrite?'))postform(document.form1,'skilllist.php?a=s&f=<?=$HTTP_GET_VARS[f]?>&wid=<?=$wid?>')">
</form>
</body>
</html>
