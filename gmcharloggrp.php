...
if($wid)
{
    ...
    */
    ...
    if ($gmchar_var == "1")
    {
        $html_save = "<input type=\"button\" name=\"Button\" value=\"Save\" onClick=\"var save=false;with(document.form1){for(var n=0;n<elements('affected[]').length;n++){save=elements('affected[]').item(n).checked;if(save)break}if(!save){alert('no change');return 0} if(confirm('Overwrite?'))postform(document.form1,'gmcharloggrp.php?a=s&i=$HTTP_GET_VARS[i]')}\">";
    }
    elseif ($gmchar_var == "0")
    {
...
<hr>
  ...
  <table border="1" cellspacing=0>
    <tr>
      ...
      <td> <input name="LogMessage_<?=$i?>" type="text" maxlength=200 size=100 value="<?=$row_grps[logmsg]?>" onchange="document.form1.elements('affected[]').item(<?=$idx?>).checked=1" <?=$readonly_flag?>></td>
    ...
    <td>
      <?
			...
			?>
			<input type="checkbox" name="Groups<?=$row_grp[groups]?>_<?=$i?>"   onclick="document.form1.elements('affected[]').item(<?=$idx?>).checked=1" <?=$grp_status?> <?=$disabled_flag?>>
