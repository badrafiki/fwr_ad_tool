...
else
{
	...
	$stancelist_table = "stancelist_" . ($HTTP_GET_VARS[f] % 10);
	...
	if($HTTP_GET_VARS[a]=="s")
	{
		...
		if($HTTP_POST_VARS["ids"])
		{
			...
			foreach($indxes as $indx)
			{
				...
				$stanceid=$HTTP_POST_VARS["stanceid_$indx"];
				$rank=$HTTP_POST_VARS["rank_$indx"];

				$query_rs = "UPDATE $stancelist_table SET
					StanceID='{$stanceid}',
					Rank='{$rank}'
					WHERE Indx='$indx'";

				$befores = get_str_rs($dbWc, "SELECT p.Username, p.CharacterName, s.* FROM $stancelist_table s, pcharacter p WHERE Indx='$indx' AND s.CharID=p.CharID;");
				$rs = mysql_query($query_rs, $dbWc) or die(mysql_error($dbWc));
				$after = get_str_rs($dbWc, "SELECT p.Username, p.CharacterName, s.* FROM $stancelist_table s, pcharacter p WHERE Indx='$indx' AND s.CharID=p.CharID;");
				
			}
		}
		header("Location: stancelist.php?f={$HTTP_GET_VARS[f]}&wid=$wid");
		exit();
	}

	$query_rs = "SELECT * FROM $stancelist_table WHERE CharID='{$HTTP_GET_VARS[f]}' ORDER BY StanceID DESC";
...
<body>
<h3>Player Character Stance List</h3>
...
?>
    <tr>
      ...
      <td><input name="stanceid_<?=$row[Indx]?>" type="text" id="stanceid_<?=$row[Indx]?>" value="<?=$row[StanceID]?>" size="8" onchange="saveid(<?=$row[Indx]?>)"><!--a href="stances.php?i=<?=$row[StanceID]?>&wid=<?=$wid?>"--><?=getstring($row[StanceID],'stance')?><!--/a--></td>
	...
	<input type="hidden" name="mark_<?=$row[Indx]?>" value="">
      ...
      <!--td><input type="button" name="Button" value="Save" onClick="if(confirm('Overwrite?'))postform(document.form1,'stancelist.php?a=s&f=<?=$HTTP_GET_VARS[f]?>&i=<?=$row[Indx]?>&wid=<?=$wid?>')"></td-->
    </tr>
    <?php
}
?>
...
<input type="reset" value="Reset"> <input type="button" name="Button" value="Save" onClick="if(confirm('Overwrite?'))postform(document.form1,'stancelist.php?a=s&f=<?=$HTTP_GET_VARS[f]?>&wid=<?=$wid?>')">
  </form>
</body>
</html>
