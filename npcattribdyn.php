...
else
{
	...
	{
		$query_rs = "UPDATE npcattribdyn SET IsDead= '{$HTTP_POST_VARS[IsDead]}' WHERE AttribID='{$HTTP_GET_VARS[i]}'";

		$befores = get_str_rs($dbWc, "SELECT * FROM npcattribdyn  WHERE AttribID='{$HTTP_GET_VARS[i]}'");
		$rs = mysql_query($query_rs, $dbWc) or die(mysql_error($dbWc));
		$after = get_str_rs($dbWc, "SELECT * FROM npcattribdyn  WHERE AttribID='{$HTTP_GET_VARS[i]}'");
		
		header ("Location: npcattribdyn.php?i={$HTTP_GET_VARS[i]}&wid=$wid");
		exit;
	}
	...
	$query_rs = "SELECT * FROM npcattribdyn WHERE AttribID='{$HTTP_GET_VARS[i]}'";
...
<br><br>
  <table border="1" cellspacing=0>
    ...
    <tr>
      <td>IsDead</td>
      <td><select name='IsDead' type='text' value='<?=$row[IsDead]?>'>
<?
	$selected_0 = $selected_1 = "";
	eval("\$selected_{$row[IsDead]} = 'SELECTED';");
...
?>
    ...
    <input type="button" name="Button" value="Save" onClick="if(confirm('Overwrite?'))postform(document.form1,'npcattribdyn.php?a=s&i=<?=$HTTP_GET_VARS[i]?>&wid=<?=$wid?>')">
</form>
</body>
</html>
