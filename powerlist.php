<?php
//die($HTTP_SERVER_VARS['REQUEST_URI']);
...
else
{
	...
	$powerlist_table = "powerlist_" . ($HTTP_GET_VARS[f] % 10);
	...
	if($HTTP_GET_VARS[a]=="s")
	{
		...
		{
			echo "<form name=form1 action=\"{$HTTP_SERVER_VARS[REQUEST_URI]}&force=1&wid=$wid\" method='Post'>";
		...
		if($HTTP_POST_VARS["ids"])
		{
			...
			foreach($indxes as $indx)
			{
				...
				$powerid=$HTTP_POST_VARS["powerid_$indx"];
				$rank=$HTTP_POST_VARS["rank_$indx"];

				$query_rs = "UPDATE $powerlist_table SET
					PowerID='{$powerid}',
					Rank='{$rank}'
					WHERE Indx='$indx'";

				$befores = get_str_rs($dbWc, "SELECT p.Username, p.CharacterName, po.* FROM $powerlist_table po, pcharacter p WHERE Indx='$indx' AND p.CharID=po.CharID;");
				$rs = mysql_query($query_rs, $dbWc) or die(mysql_error($dbWc));
				$after   = get_str_rs($dbWc, "SELECT p.Username, p.CharacterName, po.* FROM $powerlist_table po, pcharacter p WHERE Indx='$indx' AND p.CharID=po.CharID;");
				
			}
		}
		header("Location: powerlist.php?f={$HTTP_GET_VARS[f]}&wid=$wid");
		exit();
	}

	$query_rs = "SELECT * FROM $powerlist_table WHERE CharID='{$HTTP_GET_VARS[f]}' ORDER BY PowerID DESC";
...
<body>
<h3>Player Character Power List</h3>
...
?>
    <tr>
      ...
      <td><input name="powerid_<?=$row[Indx]?>" type="text" id="powerid_<?=$row[Indx]?>" value="<?=$row[PowerID]?>" size="8" onchange="saveid(<?=$row[Indx]?>)"><!--a href="powers.php?i=<?=$row[PowerID]?>&wid=<?=$wid?>"--><?=getstring($row[PowerID],'power')?><!--/a--></td>
      <td><input name="rank_<?=$row[Indx]?>" type="text" id="rank_<?=$row[Indx]?>" value="<?=$row[Rank]?>" size="8" onchange="saveid(<?=$row[Indx]?>)">
	<input type="hidden" name="mark_<?=$row[Indx]?>" value="">
</td>
      <!--td><input type="button" name="Button" value="Save" onClick="if(confirm('Overwrite?'))postform(document.form1,'powerlist.php?a=s&f=<?=$HTTP_GET_VARS[f]?>&i=<?=$row[Indx]?>&wid=<?=$wid?>')"></td-->
    </tr>
    <?php
}
?>
...
<input type="reset" value="Reset"> <input type="button" name="Button" value="Save" onClick="if(confirm('Overwrite?'))postform(document.form1,'powerlist.php?a=s&f=<?=$HTTP_GET_VARS[f]?>&wid=<?=$wid?>')">
</form>
</body>
</html>
